...
//   color: text (nullable, default: '#126eda'::text)
...
//   color: text (not null, default: '#126eda'::text)
...
//   CREATE OR REPLACE FUNCTION public.admin_create_user(new_email text, new_password text, full_name text, plan text DEFAULT 'basic'::text, profile_type text DEFAULT 'personal'::text)
//    RETURNS uuid
//    LANGUAGE plpgsql
//    SECURITY DEFINER
...
// --- TRIGGERS ---
// Table: profiles
//   on_profile_plan_update: CREATE TRIGGER on_profile_plan_update BEFORE UPDATE OF plan ON public.profiles FOR EACH ROW EXECUTE FUNCTION enforce_team_plan_restriction()
