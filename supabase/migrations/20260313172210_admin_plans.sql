...
CREATE TRIGGER on_profile_plan_update
  ...
  FOR EACH ROW EXECUTE FUNCTION public.enforce_master_team_plan_restriction();
...
CREATE OR REPLACE FUNCTION public.admin_create_user(
  ...
  new_password text,
  full_name text,
  plan text DEFAULT 'solo'::text,
  profile_type text DEFAULT 'personal'::text
) RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
...
SET plan = 
  CASE 
    WHEN plan = 'basic' THEN 'solo'
    WHEN plan = 'team' THEN 'master_team'
    WHEN plan LIKE '%pro%' THEN 'pro'
    WHEN plan LIKE '%plus%' THEN 'duo'
    WHEN plan LIKE '%max%' THEN 'business'
    ELSE plan
  END;
